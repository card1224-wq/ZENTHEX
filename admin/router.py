...
def get_launch_review(Authorization: str = Header(None), db: Session = Depends(get_db)):
    ...
    checks = [
        ...
        _review_item("logged_in_home_nav", "Logged-in homepage navigation", all(text in index_html for text in ["마이페이지", "고객센터", "로그아웃", "Studio 작업실", "Trading 엔진"]), "Logged-in users see account, support, logout, and workroom actions."),
        ...
        _review_item("operating_cost_review", "Operating cost review", all(text in master_plan for text in ["비용 단계", "무료 PostgreSQL", "정식 유료 서비스", "결제 수수료", "GPU Worker"]), "CEO review separates validation costs from production operating costs."),
        _review_item("master_plan", "Master plan document", all(text in master_plan for text in ["Zenthex SaaS Master Plan", "데이터 보존 원칙", "자동결제", "CEO 대시보드", "최종 결론"]), "Current architecture, role separation, billing, data retention, and launch risks are documented."),
